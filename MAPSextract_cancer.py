...
# an example of a more complicated where the subset can only be obtained by complementarity
subset = 'cancer'
d = hl.eval(ht.globals.freq_index_dict)
isubset = d['non_cancer']

print(f'in subset {subset}')

ht = ht.group_by('context', 'ref', 'alt', 'methylation_level', 'protein_coding', 'worst_csq').aggregate(variant_count=hl.agg.count_where((ht.freq[0].AC - ht.freq[isubset].AC) > 0), singleton_count=hl.agg.count_where((ht.freq[0].AC - ht.freq[isubset].AC) == 1))
