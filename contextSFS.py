...
## restrict to synonymous variants early for performance
ht = ht.filter(ht.worst_csq == 'synonymous_variant').persist()

context = context_ht[ht.key]
ht = prepare_ht(ht.annotate(context=context.context, methylation=context.methylation), True, False)

print('Building SFS 1..127 and binned after')

ht = ht.annotate(cat = hl.if_else(ht.freq[0].AC < 128, ht.freq[0].AC, hl.floor(hl.log(ht.freq[0].AC, 2)) + 121 )
# count variants
# manually since count_variants fails
# 214 is index for gnomAD controls
ht = ht.group_by('context', 'ref', 'alt', 'methylation_level', 'protein_coding', 'worst_csq', 'cat').aggregate(variant_count=hl.agg.count())
...
syn_ps_ht.export('gs://20211021-maps/syn_sfs_spark.tsv')